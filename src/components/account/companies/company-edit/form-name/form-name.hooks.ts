...
import { FORM_NAME_INITIAL_VALUES } from 'components/account/company/company-edit/form-name/form-name.constants';
import { FormNameValues } from 'components/account/company/company-edit/form-name/form-name.types';
...
export const useFormName: UseFormBase<FormNameValues> = (onSubmit, values) => {
  ...
  const validation: ValidationRules<FormNameValues> = {
    name: {
      required: {
        value: true,
        message: t('account:company.form.name.errors.empty'),
      },
    },
  };
